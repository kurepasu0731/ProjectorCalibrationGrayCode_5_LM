...
void GRAYCODE::calcParameters(int num){
...
//	changePoints(points_pro, points_3d);
	...
	//six_points_calibration(points_3d, points_pro, pro_param.perspectiveMat); //元祖
	six_points_calibration_2(points_3d, points_pro, pro_param.perspectiveMat); //RoomAlive参考にしてRANSACで対応点を振るうVer.
	six_points_calibration_3(points_3d, points_pro, pro_param.perspectiveMat, num, 10000); //6pointsにかける点数を可変にして自己流で対応点を振るったVer.

	//Matのサイズが取れているか
	std::cout << "perspectiveMat = " << pro_param.perspectiveMat.size() << std::endl;

	//結果の表示(取得した対応点全点での再投影誤差)
	std::cout << "*          all reprojection error average = " << 
		inspection_error_value(pro_param.perspectiveMat, points_3d, Size(800, 600), points_pro)
...
void GRAYCODE::OptimizeParameters()
{
	...
	//元祖 ↓対応点全部突っ込むとすごく時間が掛かるので、RANSAC的な感じで削減する必要あり
//	getper::get_perspectiveMat(points_pro, points_3d, pro_param.perspectiveMat, pro_param.perspectiveMat);

	//従来(Gauss-Newton)
	//getper::get_perspectiveMat(g_imagePointInlierSet, g_worldPointInlierSet, pro_param.perspectiveMat, pro_param_Gauss.perspectiveMat);
	////結果の表示(Gauss-Newton)
	//std::cout << "*                result by Gauss = " << 
	//	inspection_error_value(pro_param_Gauss.perspectiveMat, points_3d, Size(800, 600), points_pro)
	//	<<std::endl;

	//LM法
	getper::get_perspectiveMat_LM(&g_imagePointInlierSet, &g_worldPointInlierSet, pro_param.perspectiveMat, pro_param_LM.perspectiveMat);
	//結果の表示(LM法)
	std::cout << "*                all points reprojection error ave by LM = " << 
		inspection_error_value(pro_param_LM.perspectiveMat, points_3d, Size(800, 600), points_pro)
		<<std::endl;

	//結果の保存
	pro_param.alpha = 1.0;
	pro_param.beta = 0.0;
	pro_param_Gauss.alpha = 1.0;
	pro_param_Gauss.beta = 0.0;
	pro_param_LM.alpha = 1.0;
	pro_param_LM.beta = 0.0;

	std::stringstream stream;
	stream << random_calib_2d_points.size(); //初期値で使用した点数を末尾に付ける

	//saveCameraParams("../projector_paramater_Gauss" + stream.str() + ".xml", pro_param_Gauss);
	saveCameraParams("../projector_paramater_LM" +stream.str() + ".xml", pro_param_LM);
}


//プロジェクタの歪み係数を求める
void GRAYCODE::EstimateProjectorDistortionParameter()
{
}
...
//ランダムにnum点を抽出
void GRAYCODE::get_random_points(vector<Point2d>& calib_p, vector<Point3d>& calib_P, vector<Point2d>& src_p, vector<Point3d>& src_P, int num){
	int i=0;
	//初期化
	calib_p.clear();
	calib_P.clear();
...
double GRAYCODE::inspection_error_value(Mat& cameraMat, vector<Point3d>& P, Size& imageSize, vector<Point2d>& groundTruth){
...
void GRAYCODE::reProjectPoints(int radius)
{
	...
	for(int i = 0; i < g_worldPointInlierSet.size(); i++)
	{
		...
		//描画
		circle(reProjectImage, g_imagePointInlierSet[i], radius, cv::Scalar(0,0,255), -1, CV_AA); //正解 赤
		circle(reProjectImage, reprojectPoint, radius, cv::Scalar(255,0,0), -1, CV_AA); //再投影 青
	}
...
void GRAYCODE::six_points_calibration(vector<Point3d>& points_3d, vector<Point2d>& points_2d, Mat& dst){
	...
	for(int i=0; i<10; i++){
		...
		ave = inspection_error_value(pm, points_3d, Size(512, 424), points_2d);
...
void GRAYCODE::six_points_calibration_2(vector<Point3d>& points_3d, vector<Point2d>& points_2d, Mat& dst){
	...
	for(int i=0; (numCompletedFits < 4) && (i < 10); i++){
		...
		//num点を非同一平面になるように取ってくる
		while(!nonCoplanar)
		{
			get_random_points(calib_2d_points, calib_3d_points, points_2d, points_3d, 100);
		...
		//6pointsAlgorithmをnum 点でやるバージョン
		six_points_algolism_2(calib_3d_points, calib_2d_points, pm, 6);

		error = inspection_error_value(pm, calib_3d_points, Size(512, 424), calib_2d_points);
		cout << "6 points error =" << error << endl;
		...
		if(eniughInliers)
		{
			...
			double error2 = inspection_error_value(pm, worldPointInlierSet, Size(512, 424), imagePointInlierSet);
...
//自己流で対応点をふるいにかけて,6pointsをnum点でやる
void GRAYCODE::six_points_calibration_3(vector<Point3d>& points_3d, vector<Point2d>& points_2d, Mat& dst, int num, int randomPointsNum){
	...
	//6pointsを10回繰り返して一番再投影誤差が小さいパラメータpmを初期値とする
	for(int i=0; i<10; i++){
		Mat sixPointResult_pm;
		double ave;

		get_random_points(calib_2d_points, calib_3d_points, points_2d, points_3d, num);

		six_points_algolism_2(calib_3d_points, calib_2d_points, sixPointResult_pm, num);
		//num 点での再投影誤差
		int six_point_error = inspection_error_value(sixPointResult_pm, calib_3d_points, Size(512, 424), calib_2d_points);
		std::cout << num << " points error ave =" << six_point_error << std::endl;

		ave = inspection_error_value(sixPointResult_pm, points_3d, Size(512, 424), points_2d);
		std::cout << i << " times ave = " << ave << std::endl;
		if(ave < min_ave){
			dst = sixPointResult_pm;
			min_ave = ave;

			//初期値を求めるのに使ったnum点を描画用(緑)に保存
			random_calib_2d_points = calib_2d_points;
			random_calib_3d_points = calib_3d_points;
		}
	}
	...
	//初期化
	g_imagePointInlierSet.clear();
	g_worldPointInlierSet.clear();

	//1.全対応点の中からランダムにrandomPointsNum点選ぶ
	//get_random_points(g_imagePointInlierSet, g_worldPointInlierSet, points_2d, points_3d, randomPointsNum);

	//2.初期値での再投影誤差が1.0f以下の点を抽出
	double sumError = 0;
	int poinsInSum = 0;
	Mat pm = dst; //初期値
	...
	for(int i=0; i<(int)points_3d.size(); i++){
		...
		double y = (pm.at<double>(1,0)*points_3d[i].x + pm.at<double>(1,1)*points_3d[i].y + pm.at<double>(1,2)*points_3d[i].z + pm.at<double>(1,3))
			...
			Point2d p(x,y);
 			double thisError = pow(pow(points_2d[i].x-p.x,2.0)+pow(points_2d[i].y-p.y,2.0),0.5);
			double errorAbs = sqrt(thisError);
			if( errorAbs < 1.0f) //再投影誤差が許容範囲内ならば正対応点とする
			{
				worldPointInlierSet.push_back(points_3d[i]);
				imagePointInlierSet.push_back(points_2d[i]);
	...
	std::cout << "Inlier points: " << poinsInSum  << "--- ave error: " <<sumError/poinsInSum << std::endl;
}
