...
public:
	...
	//最適化の結果
	Params pro_param_Gauss; //ガウスニュートン
	Params pro_param_LM; //LM法
...
//	bool six_points_algolism(vector<Point3d>& P, vector<Point2d>& p, Mat& cameraMat);
	...
	//パラメータ推定開始
	void calcParameters(int num);
	//最適化
	void OptimizeParameters();
	//プロジェクタの歪み係数推定
	void EstimateProjectorDistortionParameter();
...
private:
	...
	//自己流で対応点をふるって、6pointsをnum点でやるバージョン
	void six_points_calibration_3(vector<Point3d>& points_3d, vector<Point2d>& points_2d, Mat& dst, int num, int randomPointsNum);
	//ランダムにnum点を抽出
	void get_random_points(vector<Point2d>& calib_p, vector<Point3d>& calib_P, vector<Point2d>& src_p, vector<Point3d>& src_P, int num);
	//互換変換
	...
	double inspection_error_value(Mat& cameraMat, vector<Point3d>& P, Size& imageSize, vector<Point2d>& groundTruth);
	//平面フィッティング
